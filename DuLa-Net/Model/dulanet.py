...
class DuLaNet(nn.Module):
    def __init__(self, backbone):
        ...
        self.e2p = E2P(cf.pano_size, cf.fp_size, cf.fp_fov, gpu=False)

        fuse_dim = [int((cf.pano_size[0]/32)*2**i) for i in range(6)]
        self.e2ps_f = [E2P((n, n*2), n, cf.fp_fov, gpu=False) for n in fuse_dim]
...
if __name__ == '__main__':
    ...
    model = DuLaNet(backbone='resnet50').to(device)
    batch = torch.ones(4, 3, 512, 1024).to(device)
