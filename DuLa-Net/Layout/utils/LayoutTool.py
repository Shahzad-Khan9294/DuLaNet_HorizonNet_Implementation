...
def calcLayoutPointType(scene):
    ...
    for i in range(len(walls)):
        ...
        # 🔒 safety check: skip if invalid intersection
        if pc is None or p1 is None or p2 is None:
            continue
...
def genLayoutEdgeMap(scene, size, dilat=4, blur=20):
    ...
    for i in range(edgeMap.shape[2]):
        max_val = edgeMap[:,:,i].max()
        edgeMap[:,:,i] = edgeMap[:,:,i] / max_val if max_val > 0 else edgeMap[:,:,i]
...
def genLayoutObj2dMap(scene, size):
    ...
    for obj2d in scene.layoutObjects2d:
